...
function loadMobileCss(){
  ...
  const l=document.createElement('link');l.rel='stylesheet';l.href=assetPath('mobile.css?v=20260820-2');l.dataset.setariMobile='1';document.head.appendChild(l);
}
function ensureBrandLogo(){
  document.querySelectorAll('.brand').forEach(b=>{
    let img=b.querySelector('img.brand-logo-img, img[src*="logo-setari"]');
    if(!img){img=document.createElement('img');img.src=assetPath('logo-setari.svg?v=20260820-2');img.alt='SETARI — Science, Engineering, Technology, Applied Research & Innovation';img.className='brand-logo-img';b.prepend(img)}else img.classList.add('brand-logo-img');
...
function addEditorialScript(file,key){
  ...
  const s=document.createElement('script');s.src=assetPath(file+'?v=20260820-3');s.defer=true;s.dataset['setari'+key.charAt(0).toUpperCase()+key.slice(1)]='1';document.head.appendChild(s);
}
...
function loadI18n(){
  ...
  const s=document.createElement('script');s.src=assetPath('i18n-v2.js?v=20260820-3');s.defer=true;s.dataset.setariI18nV2='1';
  s.onload=()=>{const f=document.createElement('script');f.src=assetPath('i18n-fixes.js?v=20260820-3');f.defer=true;f.dataset.setariI18nFixes='1';f.onload=()=>{if(window.SETARI_I18N?.apply)window.SETARI_I18N.apply();if(window.SETARI_I18N_FIXES?.apply)window.SETARI_I18N_FIXES.apply();};document.head.appendChild(f)};
