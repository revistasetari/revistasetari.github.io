...
function ensureBrandLogo(){
  document.querySelectorAll('.brand').forEach(b=>{
    ...
    if(!img){img=document.createElement('img');img.src=assetPath('logo-setari.svg?v=20260818-3');img.alt='SETARI — Science, Engineering, Technology, Applied Research & Innovation';img.className='brand-logo-img';b.prepend(img)}else img.classList.add('brand-logo-img');
...
function loadI18n(){if(window.SETARI_I18N)return;if(document.querySelector('script[data-setari-i18n-v2]'))return;const s=document.createElement('script');s.src=assetPath('i18n-v2.js?v=20260818-3');s.defer=true;s.dataset.setariI18nV2='1';document.head.appendChild(s)}
