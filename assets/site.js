...
function ensureBrandLogo(){
  document.querySelectorAll('.brand').forEach(b=>{
    ...
    if(!img){img=document.createElement('img');img.src=assetPath('logo-setari.svg?v=20260819-1');img.alt='SETARI — Science, Engineering, Technology, Applied Research & Innovation';img.className='brand-logo-img';b.prepend(img)}else img.classList.add('brand-logo-img');
...
function loadAndrei(){
  ...
  if(document.querySelector('script[data-setari-andrei]'))return;
  const a=document.createElement('script');a.src=assetPath('andrei-bonamigo.js?v=20260819-1');a.defer=true;a.dataset.setariAndrei='1';document.head.appendChild(a);
}
function loadI18n(){
  if(document.querySelector('script[data-setari-i18n-v2]'))return;
  const s=document.createElement('script');s.src=assetPath('i18n-v2.js?v=20260819-1');s.defer=true;s.dataset.setariI18nV2='1';
  s.onload=()=>{const f=document.createElement('script');f.src=assetPath('i18n-fixes.js?v=20260819-1');f.defer=true;f.dataset.setariI18nFixes='1';f.onload=loadAndrei;document.head.appendChild(f)};
