(()=>{
if(window.__SETARI_PORTAL_I18N_DIRECT__)return;
window.__SETARI_PORTAL_I18N_DIRECT__=true;
const KEY='setariLang';
let lang=localStorage.getItem(KEY)==='en'?'en':'pt';
const pairs=[
...
['Artigo submetido com sucesso. A identidade do autor ficou separada do manuscrito.','Manuscript submitted successfully. Author identity has been kept separate from the manuscript.'],['Parecer enviado ao Editor-Chefe.','Review submitted to the Editor-in-Chief.'],['Função atualizada.','Role updated.'],['Nenhuma submissão ainda.','No submissions yet.'],['Nenhum artigo atribuído a você.','No manuscripts assigned to you.'],['Nenhuma submissão recebida.','No submissions received.'],['Nenhum usuário.','No users.'],['Carregando…','Loading…'],['Sem nome','No name'],['pendente','pending'],['Arquivo de parecer anotado (opcional)','Annotated review file (optional)'],['Baixar arquivo do parecer','Download review file']
];
function applyStatic(){
 ...
 document.querySelectorAll('[data-pt][data-en]').forEach(el=>{
  ...
  if(v==null)return;
  if(el.tagName==='INPUT'||el.tagName==='TEXTAREA')el.value=v;else el.innerHTML=v;
 });
 ...
 document.querySelectorAll('[data-lang-toggle]').forEach(b=>{
  ...
  b.setAttribute('aria-label',lang==='en'?'Mudar idioma para português':'Switch language to English');
 });
}
...
function applyDynamic(root=document.body){
 if(!root)return;
...
function bind(){
 document.querySelectorAll('[data-lang-toggle]').forEach(b=>{
  b.onclick=changeLanguage;
  b.style.pointerEvents='auto';
  b.removeAttribute('disabled');
 });
}
function boot(){apply();bind();setTimeout(()=>{apply();bind()},500);setTimeout(()=>{apply();bind()},1500)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.SETARI_PORTAL_I18N={apply,lang:()=>lang,toggle:changeLanguage};
})();